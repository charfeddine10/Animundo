    ...
    useCallback(() => {
      ...
      const loadProfile = async () => {
        try {
          ...
          setFavoriteCount(watchlist.filter((item) => item.favorite).length);

          setCompletedCount(watchlist.filter((item) => item.completed).length);
          ...
          {recentAnime.length === 0 ? (
            ...
            recentAnime.map((item, index) => (
              ...
              >
                ...
                <Text className="text-white ml-3 flex-1">{item.title}</Text>
              </TouchableOpacity>
            ))
          )}
        </View>
